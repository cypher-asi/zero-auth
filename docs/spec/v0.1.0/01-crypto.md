# zid-crypto Specification v0.1

## 1. Overview

The `zid-crypto` crate provides all cryptographic primitives for the Zero-Auth system. It is the most fundamental crate in the dependency hierarchy, with no internal dependencies on other Zero-Auth crates.
...
The root cryptographic seed from which all identity keys are derived. This is the most sensitive key in the system.
...
impl NeuralKey {
    ...
    /// Input bytes are zeroized after copying
    pub fn from_bytes(bytes: [u8; 32]) -> Self;
    
    /// Get a reference to the key bytes
    /// WARNING: Use with extreme caution. Never log or persist.
...
X25519 encryption key pair for key agreement (Diffie-Hellman).
...
> **Note**: These are distinct from the policy operation requirements in `zid-policy`, which define what permissions are needed to perform specific API operations. Machine key capabilities are about what the key *can do*; policy capabilities are about what's *required* for an operation.
...
All key derivations use HKDF-SHA256 with domain separation strings.
...
/// Derive Machine signing seed from machine seed
...
/// Derive Machine encryption seed from machine seed
...
/// High-level: Derive complete Machine Key pair
...
```rust
/// Canonicalize identity creation message (137 bytes)
...
/// Canonicalize machine enrollment message (109 bytes)
...
/// Canonicalize recovery approval message (73 bytes)
...
/// Canonicalize rotation approval message (57 bytes)
...
/// Canonicalize challenge for signing (130 bytes)
...
```rust
/// Hash data using BLAKE3 (fast, non-cryptographic use)
pub fn blake3_hash(data: &[u8]) -> [u8; 32];

/// Derive key ID from public key (first 16 bytes of BLAKE3 hash)
...
```rust
/// A Neural Shard - one share of a split Neural Key
pub struct NeuralShard {
    pub index: u8,        // 1-255 (we use 1-5)
...
```rust
/// Generate cryptographically random nonce (24 bytes)
pub fn generate_nonce() -> Result<[u8; 24]>;

/// Generate challenge nonce (32 bytes)
pub fn generate_challenge_nonce() -> Result<[u8; 32]>;
...
sequenceDiagram
    ...
    Note over Crypto: HKDF("cypher:shared:machine:v1" || identity_id || machine_id || epoch)
    Note over Crypto: Step 2: derive_machine_signing_seed()
    Note over Crypto: HKDF("cypher:shared:machine:sign:v1" || machine_id)
    Note over Crypto: Step 3: derive_machine_encryption_seed()
    Note over Crypto: HKDF("cypher:shared:machine:encrypt:v1" || machine_id)
...
#### Neural Key Rotation Approval (57 bytes)
...
1. **Neural Key Entropy**: Must not be all zeros or single repeated byte
2. **Nonce Uniqueness**: Each nonce MUST be unique per key (random generation provides statistical uniqueness)
3. **Shard Indices**: Must be non-zero and unique when combining
4. **Timestamps**: All timestamps are Unix seconds in big-endian format
...
### 6.5 Threat Mitigations

| Threat | Mitigation |
|--------|------------|
| Key extraction from memory | Zeroization on drop |
| Timing attacks | Constant-time signature verification, constant-time compare |
| Nonce reuse | 192-bit random nonces (birthday bound at 2^96) |
| Rainbow tables | Argon2id with random salt |
| Single point of failure | 3-of-5 Shamir for Neural Key |
...
|-------|---------|---------|
| `chacha20poly1305` | workspace | XChaCha20-Poly1305 AEAD |
| `hkdf` | workspace | HKDF-SHA256 key derivation |
| `sha2` | workspace | SHA-256 for HKDF |
| `ed25519-dalek` | workspace | Ed25519 signatures |
| `x25519-dalek` | workspace | X25519 key agreement |
