# zero-auth-server Specification v0.1

## 1. Overview

The `zero-auth-server` crate provides the HTTP API server for Zero-Auth. It exposes RESTful endpoints for identity management, authentication, session management, and service integrations using the Axum web framework.
...
pub struct Config {
    ...
    pub database_path: PathBuf,             // Default: ./data/zero-auth.db
    pub service_master_key: [u8; 32],       // Required in prod, random in dev
    pub jwt_issuer: String,                 // Default: https://zero-auth.cypher.io
...
| `DATABASE_PATH` | No | `./data/zero-auth.db` | RocksDB path |
...
| Crate | Purpose |
|-------|---------|
| `zero-auth-crypto` | Signature verification, timestamps |
| `zero-auth-storage` | RocksDB storage backend |
| `zero-auth-policy` | Rate limiting, policy evaluation |
| `zero-auth-identity-core` | Identity and machine operations |
| `zero-auth-methods` | Authentication methods |
| `zero-auth-sessions` | Session and token management |
| `zero-auth-integrations` | Service registration, SSE |
