...
use uuid::Uuid;
use zid_crypto::blake3_hash;
use zid_integrations::{
...
pub async fn event_stream(
    ...
    let stream = tokio_stream::wrappers::ReceiverStream::new(rx).map(
        |revocation_event: zid_integrations::RevocationEvent| {
            // Convert to JSON
            let event_json =
...
fn parse_scopes(scope_strings: &[String]) -> Result<Vec<zid_integrations::Scope>, ApiError> {
    use zid_integrations::Scope;
