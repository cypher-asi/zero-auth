...
use zid_crypto::derive_jwt_signing_seed;
use zid_identity_core::IdentityCore;
use zid_storage::{column_families::*, Storage};
...
impl<S: Storage, I: IdentityCore, E: EventPublisher> SessionService<S, I, E> {
    ...
    pub(super) async fn generate_signing_key(&self, epoch: u64) -> Result<JwtSigningKey> {
        ...
        let nonce = zid_crypto::generate_nonce()?;
        let private_key_encrypted = zid_crypto::encrypt_jwt_signing_key(
