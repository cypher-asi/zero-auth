...
fn render_enroll_dialog(ui: &mut Ui, state: &mut AppState, rt: &tokio::runtime::Handle) {
    ...
    egui::Window::new("Enroll New Machine")
        ...
        .show(ui.ctx(), |ui| {
            ui.label(
                RichText::new("Enter your passphrase to derive a new machine keypair.")
            ...
            ui.horizontal(|ui| {
                ...
                ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                    ...
                    if core::primary_button(ui, "Enroll", can) {
                        ...
                        rt.spawn(async move {
                            let nonce: [u8; 24] = match creds.shards_nonce.as_slice().try_into() {
                                Ok(n) => n,
                                Err(_) => {
                                    let _ = tx.send(AppMessage::Error(crate::error::AppError::StorageError("Invalid nonce".into())));
                                    return;
                                }
                            };
                            let commitment: [u8; 32] = match creds.neural_key_commitment.as_slice().try_into() {
                                Ok(c) => c,
                                Err(_) => {
                                    let _ = tx.send(AppMessage::Error(crate::error::AppError::StorageError("Invalid commitment".into())));
                                    return;
                                }
                            };

                            let shards = match crate::service::key_shard::decrypt_device_shards(
                            ...
                            let _ = tx.send(AppMessage::Toast(ToastLevel::Info, "Machine enrollment requires a user shard. Use login flow for now.".into()));
                        });
                    }
                });
            });
