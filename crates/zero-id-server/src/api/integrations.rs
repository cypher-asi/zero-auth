...
use uuid::Uuid;
use zero_id_crypto::blake3_hash;
use zero_id_integrations::{
...
pub async fn event_stream(
    ...
    let stream = tokio_stream::wrappers::ReceiverStream::new(rx).map(
        |revocation_event: zero_id_integrations::RevocationEvent| {
            // Convert to JSON
            let event_json =
...
fn parse_scopes(scope_strings: &[String]) -> Result<Vec<zero_id_integrations::Scope>, ApiError> {
    use zero_id_integrations::Scope;
