...
use std::sync::Arc;
use zero_id_identity_core::IdentityCoreService;
use zero_id_integrations::IntegrationsService;
use zero_id_methods::AuthMethodsService;
use zero_id_policy::PolicyEngineImpl;
use zero_id_sessions::{NoOpEventPublisher, SessionService};
...
#[async_trait]
impl zero_id_identity_core::EventPublisher for IdentityNoOpPublisher {
    async fn publish(
        &self,
...
impl AppState {
    pub async fn new(config: Config) -> Result<Self> {
        ...
        let oauth_configs = zero_id_methods::OAuthConfigs {
            google: config
                .oauth_google
                .as_ref()
                .map(|c| zero_id_methods::OAuthProviderConfig {
            ...
            epic_games: config.oauth_epic.as_ref().map(|c| {
                zero_id_methods::OAuthProviderConfig {
                    client_id: c.client_id.clone(),
                    client_secret: c.client_secret.clone(),
                    redirect_uri: c.redirect_uri.clone(),
                }
