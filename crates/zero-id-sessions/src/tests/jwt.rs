...
async fn test_jwt_signing_key_generation() {
    ...
    let seed = zero_id_crypto::derive_jwt_signing_seed(&service_master_key, 1).unwrap();

    assert_eq!(seed.len(), 32);

    // Keys from different epochs should be different
    let seed2 = zero_id_crypto::derive_jwt_signing_seed(&service_master_key, 2).unwrap();
