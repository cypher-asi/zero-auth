...
use uuid::Uuid;
use zero_id_identity_core::{IdentityCore, IdentityCoreService};
use zero_id_policy::PolicyEngineImpl;
...
#[async_trait]
impl zero_id_identity_core::EventPublisher for MockIdentityCoreEventPublisher {
    async fn publish(
        &self,
...
pub async fn create_test_session_service(
    ...
    let issuer = "zero-id.test".to_string();
...
pub async fn create_test_identity_with_machine(
    ...
    use zero_id_crypto::{canonicalize_identity_creation_message, sign_message, Ed25519KeyPair};
    use zero_id_identity_core::{CreateIdentityRequest, MachineKey};
    ...
    let machine_key = MachineKey {
        ...
        capabilities: zero_id_crypto::MachineKeyCapabilities::FULL_DEVICE,
        epoch: 1,
        created_at: now,
        expires_at: None,
        last_used_at: None,
