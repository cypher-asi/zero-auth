...
mod tests {
    ...
    use std::sync::RwLock;
    use zid_storage::{Batch, StorageError};
    ...
    impl Storage for MockStorage {
        async fn get<K, V>(&self, cf: &str, key: &K) -> zid_storage::Result<Option<V>>
        where
        ...
        async fn put<K, V>(&self, cf: &str, key: &K, value: &V) -> zid_storage::Result<()>
        where
            K: serde::Serialize + Send + Sync,
        ...
        async fn delete<K>(&self, cf: &str, key: &K) -> zid_storage::Result<()>
        where
            K: serde::Serialize + Send + Sync,
        {
            let key_bytes = bincode::serialize(key)
        ...
        async fn exists<K>(&self, cf: &str, key: &K) -> zid_storage::Result<bool>
        where
            K: serde::Serialize + Send + Sync,
        {
            let key_bytes = bincode::serialize(key)
        ...
        async fn get_by_prefix<K, V>(
            ...
            _cf: &str,
            _prefix: &K,
        ) -> zid_storage::Result<Vec<(Vec<u8>, V)>>
        where
            K: serde::Serialize + Send + Sync,
            V: serde::de::DeserializeOwned,
        {
        ...
        async fn scan_all<V>(&self, _cf: &str) -> zid_storage::Result<Vec<(Vec<u8>, V)>>
        where
            V: serde::de::DeserializeOwned,
        {
            Ok(vec![])
        ...
        async fn begin_transaction(&self) -> zid_storage::Result<Box<dyn Batch>> {
    ...
    impl Batch for MockBatch {
        fn put_raw(&mut self, _cf: &str, _key: Vec<u8>, _value: Vec<u8>) -> zid_storage::Result<()> {
            Ok(())
        }

        fn delete_raw(&mut self, _cf: &str, _key: Vec<u8>) -> zid_storage::Result<()> {
            Ok(())
        }

        async fn commit(self: Box<Self>) -> zid_storage::Result<()> {
            Ok(())
        }
