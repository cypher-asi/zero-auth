...
use zero_id_storage::{Storage, CF_MACHINE_KEYS, CF_MACHINE_KEYS_BY_IDENTITY};
    ...
    S: Storage,
{
        ...
        for (key_bytes, _) in machines_index {
            ...
            if key_bytes.len() >= 48 {
                ...
                let machine_id = Uuid::from_slice(machine_id_bytes).map_err(|e| {
                    IdentityCoreError::Storage(zero_id_storage::StorageError::Deserialization(
                        e.to_string(),
                    ))
                })?;
    ...
    pub(super) fn verify_approval_signatures(
        ...
        use zero_id_crypto::verify_signature;
