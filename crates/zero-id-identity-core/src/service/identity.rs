...
use tracing::info;
use uuid::Uuid;
use zero_id_crypto::{canonicalize_identity_creation_message, verify_signature};
use zero_id_policy::PolicyEngine;
use zero_id_storage::{
    ...
    S: Storage + 'static,
{
    ...
    async fn validate_identity_creation_policy(
        ...
        let policy_context = zero_id_policy::PolicyContext {
            ...
            mfa_verified: false,
            operation: zero_id_policy::Operation::CreateIdentity,
            resource: Some(zero_id_policy::Resource::Identity(request.identity_id)),
            ip_address: "0.0.0.0".to_string(),
            user_agent: "zero-id-client".to_string(),
        ...
        if decision.verdict != zero_id_policy::Verdict::Allow {
    ...
    pub async fn build_policy_context(
        ...
        namespace_id: Uuid,
        operation: zero_id_policy::Operation,
        auth_method: zero_id_policy::AuthMethod,
        mfa_verified: bool,
        ip_address: String,
        user_agent: String,
    ) -> Result<zero_id_policy::PolicyContext> {
        ...
        let identity_status = identity.as_ref().map(|i| match i.status {
            IdentityStatus::Active => zero_id_policy::IdentityStatus::Active,
            IdentityStatus::Disabled => zero_id_policy::IdentityStatus::Disabled,
            IdentityStatus::Frozen => zero_id_policy::IdentityStatus::Frozen,
            IdentityStatus::Deleted => zero_id_policy::IdentityStatus::Deleted,
        });

        Ok(zero_id_policy::PolicyContext {
            identity_id,
            machine_id,
            namespace_id,
            auth_method,
            mfa_verified,
            operation,
            resource: Some(zero_id_policy::Resource::Identity(identity_id)),
            ip_address,
            user_agent,
