...
use uuid::Uuid;
use zero_id_crypto::{current_timestamp, verify_signature};
use zero_id_identity_core::{IdentityCore, IdentityStatus};
use zero_id_policy::{Operation, PolicyContext, PolicyEngine, Verdict};
use zero_id_storage::Storage;
    ...
    S: Storage,
{
    ...
    async fn check_machine_active(
        &self,
        machine_id: Uuid,
    ) -> Result<zero_id_identity_core::MachineKey> {
        let machine = self
            .identity_core
    ...
    async fn check_identity_not_frozen(
        &self,
        identity_id: Uuid,
    ) -> Result<zero_id_identity_core::Identity> {
    ...
    fn verify_challenge_signature(
        &self,
        machine: &zero_id_identity_core::MachineKey,
        challenge: &Challenge,
        response: &ChallengeResponse,
    ) -> Result<()> {
