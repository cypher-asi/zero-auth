...
A complete Rust example demonstrating how to integrate with the zid identity system.
...
Your app validates tokens by calling zid's introspection endpoint:
...
async fn get_jwks(zid_url: &str) -> Result<jsonwebtoken::jwk::JwkSet> {
    let response = reqwest::get(format!("{}/.well-known/jwks.json", zid_url)).await?;
...
async fn auth_middleware<B>(
    ...
    let response = client
        .post(format!("{}/v1/auth/introspect", state.zid_url))
...
| "Connection refused" | Make sure zid server is running on port 9999 |
...
- **zid-crypto** - Client-side cryptography (key generation, signing)
