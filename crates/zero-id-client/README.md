...
A complete Rust example demonstrating how to integrate with the zero-id identity system.
...
Your app validates tokens by calling zero-id's introspection endpoint:
...
async fn get_jwks(zero_id_url: &str) -> Result<jsonwebtoken::jwk::JwkSet> {
    let response = reqwest::get(format!("{}/.well-known/jwks.json", zero_id_url)).await?;
...
async fn auth_middleware<B>(
    ...
    let response = client
        .post(format!("{}/v1/auth/introspect", state.zero_id_url))
...
| "Connection refused" | Make sure zero-id server is running on port 9999 |
...
- **zero-id-crypto** - Client-side cryptography (key generation, signing)
