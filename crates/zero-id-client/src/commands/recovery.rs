...
async fn create_new_identity(
    ...
    let message = zero_id_crypto::canonicalize_identity_creation_message(
        &identity_id,
    ...
    let signature = zero_id_crypto::sign_message(&identity_signing_keypair, &message);
...
fn save_recovered_credentials(
    ...
    machine_keypair: &zero_id_crypto::MachineKeyPair,
    device_name: &str,
    platform: &str,
    passphrase: &str,
) -> Result<()> {
