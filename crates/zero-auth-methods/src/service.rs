    ...
    S: Storage,
{
    ...
    fn current_timestamp() -> u64 {
        std::time::SystemTime::now()
            ...
            .unwrap_or_else(|_| {
                // System time before UNIX_EPOCH is extremely unlikely in production
                // Default to 0 to avoid panic
    ...
    fn validate_password(password: &str) -> Result<()> {
        if password.len() < 12 {
            return Err(AuthMethodsError::Other("Password must be at least 12 characters long".to_string()));
        }
        
        if password.len() > 128 {
            return Err(AuthMethodsError::Other("Password must be at most 128 characters long".to_string()));
        }
        
        if !password.chars().any(|c| c.is_uppercase()) {
            return Err(AuthMethodsError::Other("Password must contain at least one uppercase letter".to_string()));
        }
        
        if !password.chars().any(|c| c.is_lowercase()) {
            return Err(AuthMethodsError::Other("Password must contain at least one lowercase letter".to_string()));
        }
        
        if !password.chars().any(|c| c.is_ascii_digit()) {
            return Err(AuthMethodsError::Other("Password must contain at least one digit".to_string()));
        }
        
        if !password.chars().any(|c| !c.is_alphanumeric()) {
            return Err(AuthMethodsError::Other("Password must contain at least one special character".to_string()));
        }
    ...
    fn validate_email(email: &str) -> Result<()> {
        ...
        if email.len() > 254 {
            return Err(AuthMethodsError::Other("Email address too long (max 254 characters)".to_string()));
        }
        
        if email.is_empty() {
            return Err(AuthMethodsError::Other("Email address cannot be empty".to_string()));
        }
        
        // Split on @ and validate parts
        let parts: Vec<&str> = email.split('@').collect();
        if parts.len() != 2 {
            return Err(AuthMethodsError::Other("Invalid email format: must contain exactly one @ symbol".to_string()));
        }
        ...
        if local.is_empty() || local.len() > 64 {
            return Err(AuthMethodsError::Other("Invalid email: local part must be 1-64 characters".to_string()));
        }
        
        // Validate domain part (after @)
        if domain.is_empty() || domain.len() > 253 {
            return Err(AuthMethodsError::Other("Invalid email: domain must be 1-253 characters".to_string()));
        }
        
        // Domain must contain at least one dot
        if !domain.contains('.') {
            return Err(AuthMethodsError::Other("Invalid email: domain must contain at least one dot".to_string()));
        }
        
        // Basic character validation
        if !local.chars().all(|c| c.is_ascii_alphanumeric() || c == '.' || c == '_' || c == '-' || c == '+') {
            return Err(AuthMethodsError::Other("Invalid email: local part contains invalid characters".to_string()));
        }
        
        if !domain.chars().all(|c| c.is_ascii_alphanumeric() || c == '.' || c == '-') {
            return Err(AuthMethodsError::Other("Invalid email: domain contains invalid characters".to_string()));
        }
    ...
    /// Internal method that completes OAuth and returns the full link
    /// This is used by both oauth_complete and authenticate_oauth to avoid redundant lookups
    async fn oauth_complete_internal(&self, request: &OAuthCompleteRequest) -> Result<OAuthLink> {
        ...
        // We do this BEFORE verification to prevent replay attacks
        self.storage
            .delete(CF_OAUTH_STATES, &request.state)
            .await?;

        // Step 2: Get OAuth config from stored configuration
        let provider_config = match request.provider {
        ...
        let provider_config = provider_config.ok_or_else(|| {
            AuthMethodsError::Other(format!("OAuth provider {:?} not configured", request.provider))
        })?;

        // Build OAuth config from environment configuration
        ...
        if let Some(existing_link) = existing_link {
            ...
            // Update last_auth_at and return the existing link
    ...
    async fn create_virtual_machine(&self, identity_id: Uuid) -> Result<Uuid> {
        use zero_auth_crypto::{hkdf_derive_32, Ed25519KeyPair};
        
        // Derive deterministic but unique keys for this identity's virtual machine
        // Using service master key ensures keys are not all zeros
        let mut ikm = Vec::with_capacity(48);
        ikm.extend_from_slice(&*self.service_master_key);
        ikm.extend_from_slice(identity_id.as_bytes());
        ...
        let encryption_keypair = zero_auth_crypto::X25519KeyPair::from_seed(&encryption_seed)?;
    ...
    S: Storage + 'static,
{
    ...
    async fn authenticate_machine(
        ...
        // Note: The challenge should have been deleted after first use,
        // so if it exists and is marked used, something went wrong
        if challenge.used {
            warn!(
                "Challenge {} already used (replay attack detected)",
                response.challenge_id
            );
            // Delete it anyway to ensure cleanup
            let _ = self.storage.delete(CF_CHALLENGES, &response.challenge_id).await;
        ...
        if self.storage.exists(CF_USED_NONCES, &nonce_key).await? {
            warn!("Challenge nonce {} already used (replay attack detected)", nonce_key);
            // Delete challenge to ensure cleanup
            let _ = self.storage.delete(CF_CHALLENGES, &response.challenge_id).await;
            return Err(AuthMethodsError::ChallengeAlreadyUsed(response.challenge_id));
        }
        
        // SECURITY FIX: Delete challenge immediately after retrieving it
        // This provides atomic "consume" semantics - second attempt will get NotFound
        // We do this BEFORE signature verification to prevent replay attacks
        self.storage
            .delete(CF_CHALLENGES, &response.challenge_id)
            .await?;
        
        // Store used nonce with expiry timestamp (for cleanup)
        // Nonces are valid for challenge TTL + 60 seconds grace period
        ...
        // Step 6: Get reputation score and evaluate policy
        let reputation_score = self.policy.get_reputation(machine.identity_id).await
    ...
    async fn authenticate_email(
        ...
        let credential_opt: Option<EmailCredential> = self
            .storage
        ...
        let (password_valid, credential) = if let Some(cred) = credential_opt {
            ...
            // This takes the same time as real verification (Argon2id computation)
            let dummy_hash = "$argon2id$v=19$m=19456,t=2,p=1$aGVsbG93b3JsZA$0123456789abcdef0123456789abcdef";
        ...
        // Step 4: Get reputation score and evaluate policy
        let reputation_score = self.policy.get_reputation(credential.identity_id).await
            .unwrap_or(50); // Default to neutral if error
    ...
    async fn verify_mfa(&self, identity_id: Uuid, code: String) -> Result<bool> {
        ...
        if self.policy.check_identity_rate_limit(identity_id).is_none() {
            ...
            let _ = self.policy.record_attempt(identity_id, Operation::Login, false).await;
            return Err(AuthMethodsError::Other("Too many MFA verification attempts. Please try again later.".to_string()));
        }
        ...
        if verify_totp_code(&secret, &code)? {
            ...
            let _ = self.policy.record_attempt(identity_id, Operation::Login, true).await;
            return Ok(true);
        }
        ...
        if mfa_secret.backup_codes.contains(&code_hash) {
            ...
            updated_secret.backup_codes.retain(|hash| hash != &code_hash);
    ...
    async fn oauth_initiate(
        ...
        // Get OAuth config from stored configuration
    ...
    async fn oauth_complete(&self, request: OAuthCompleteRequest) -> Result<Uuid> {
        let link = self.oauth_complete_internal(&request).await?;
        Ok(link.link_id)
    }
    ...
    async fn authenticate_oauth(&self, request: OAuthCompleteRequest) -> Result<AuthResult> {
        ...
        // Step 2: Verify identity is not frozen
        ...
        // Step 3: Create or get virtual machine for OAuth authentication
        let machine_id = self.create_virtual_machine(link.identity_id).await?;

        // Step 4: Get reputation score and evaluate policy
        let reputation_score = self.policy.get_reputation(link.identity_id).await
            .unwrap_or(50); // Default to neutral if error
    ...
    async fn authenticate_wallet(
        ...
        if challenge.used {
            ...
            // Delete it anyway to ensure cleanup
            let _ = self.storage.delete(CF_CHALLENGES, &signature.challenge_id).await;
        ...
        // Step 7: Get reputation score and evaluate policy
        let reputation_score = self.policy.get_reputation(credential.identity_id).await
            .unwrap_or(50); // Default to neutral if error
...
mod tests {
    ...
    fn test_derive_mfa_kek() {
        ...
        let test_service_master_key = [42u8; 32]; // Test key
        let service = AuthMethodsService::new(identity_core.clone(), policy, storage, test_service_master_key);
    ...
    async fn test_oauth_initiate() {
        ...
        // Create service with test OAuth configs
        ...
        let test_service_master_key = [42u8; 32]; // Test key
        let service = AuthMethodsService::with_oauth_configs(
            identity_core.clone(),
            policy,
            storage.clone(),
            test_service_master_key,
            oauth_configs,
        );
    ...
    async fn test_wallet_credential_attachment() {
        ...
        let test_service_master_key = [42u8; 32]; // Test key
        let service = AuthMethodsService::new(identity_core.clone(), policy, storage.clone(), test_service_master_key);
